...
def _get_net_for(date_str: str):
    nh = st.session_state.get("net_history", [])
    for r in nh:
        if r.get("date") == date_str:
            return float(r.get("net", 0.0) or 0.0)
    return None
...
if "cashflow_base_date" not in st.session_state:
    st.session_state["cashflow_base_date"] = "2026-01-28"  # varsayılan baseline
...
if st.session_state.get("force_reload_state"):
    ...
    if "net_history" in saved:
        st.session_state["net_history"] = saved["net_history"]
    if "cashflow_base_date" in saved:
        st.session_state["cashflow_base_date"] = saved["cashflow_base_date"]
...
display_df = compute_display_assets(
...
with assets_col:
    edited = st.data_editor(
        ...
        column_config={
            ...
            # NOT: aynı key'i 2 kez yazma. Tek bir "Kur (TL)" bırak.
            "Kur (TL)": st.column_config.NumberColumn(
                format="%.4f",
                step=0.0001,
                help="Otomatik yoksa manuel gir. Otomatik varsa üstüne yazar."
            ),
            ...
            "Kod": st.column_config.TextColumn(
                help="TRY, USD, EUR, GRAM, CEYREK, YARIM, ATA, BILEZIK"
            ),
        },
        disabled=["Tutar (TL)"],
        key="assets_editor",
...
debts_col, _empty = st.columns([1, 1])   # %50 borçlar, %50 boş
...
with debts_col:
    debts_df = st.data_editor(
        ...
        num_rows="dynamic",
        column_config={
            "Tutar (TL)": st.column_config.NumberColumn(step=10.0)
        },
        key="debts_editor",
    )
...
st.metric("Net (TL)", f"{net:,.2f}")





# Bilançoyu Kaydet Varlıklar ve Borçlar CSV indir Butonları
...
st.caption("Gelir - Gider = Kazanç/Zarar. Seçtiğin gün, bir önceki güne göre değişimi de gösterir. (Son 30 gün)")

cf_col, _empty = st.columns([1, 1])  # yarım genişlik + sağ boş

with cf_col:
    today = dt.date.today()
    start_day = today - dt.timedelta(days=30)

    # Takvim: sadece son 30 gün, gelecek gün seçilemez
    selected_date = st.date_input(
        "Tarih seç (son 30 gün)",
        value=today,
        min_value=start_day,
        max_value=today,
        key="cashflow_date2"   # <-- EKLE
    )

    # session'da cashflow yoksa başlat
    if "cashflow" not in st.session_state:
        st.session_state["cashflow"] = []  # [{"date":"YYYY-MM-DD","income":..,"expense":..}]

    # Kayıtları dict'e çevir (tarih -> row) hızlı erişim
    cashflow_list = st.session_state.get("cashflow", [])
    cf_map = {row.get("date"): row for row in cashflow_list if row.get("date")}

    sel_key = selected_date.isoformat()
    prev_key = (selected_date - dt.timedelta(days=1)).isoformat()

    # Seçili gün ve bir önceki gün değerlerini oku (yoksa 0)
    sel_row = cf_map.get(sel_key, {})
    prev_row = cf_map.get(prev_key, {})

    sel_income = float(sel_row.get("income", 0.0) or 0.0)
    sel_expense = float(sel_row.get("expense", 0.0) or 0.0)
    sel_net = sel_income - sel_expense

    prev_income = float(prev_row.get("income", 0.0) or 0.0)
    prev_expense = float(prev_row.get("expense", 0.0) or 0.0)
    prev_net = prev_income - prev_expense

    delta_vs_prev_day = sel_net - prev_net

    # Seçili gün için giriş alanları (default: mevcut kayıt varsa onu getirir)
    a, b = st.columns(2)
    with a:
        income = st.number_input("Gelir (TL)", min_value=0.0, value=sel_income, step=100.0, key="cf_income_sel")
    with b:
        expense = st.number_input("Gider (TL)", min_value=0.0, value=sel_expense, step=100.0, key="cf_expense_sel")

    net = float(income) - float(expense)

    # Bir önceki güne göre değişimi delta olarak göster (yeşil/kırmızı otomatik)
    if prev_key in cf_map:
        st.metric("Kazanç/Zarar (TL)", f"{net:,.2f}", delta=f"{(net - prev_net):+,.2f} (düne göre)")
    else:
        st.metric("Kazanç/Zarar (TL)", f"{net:,.2f}")
        st.info("Bir önceki gün için kayıt yok; düne göre fark hesaplanamadı.")

    # Kaydet/Güncelle
    if st.button("Seçili Günü Kaydet / Güncelle", key="cf_save_sel"):
        updated = False
        for row in cashflow_list:
            if row.get("date") == sel_key:
                row["income"] = float(income)
                row["expense"] = float(expense)
                updated = True
                break
        if not updated:
            cashflow_list.append({"date": sel_key, "income": float(income), "expense": float(expense)})

        cashflow_list.sort(key=lambda x: x.get("date", ""))
        st.session_state["cashflow"] = cashflow_list
        st.success(f"{sel_key} kaydedildi.")

    # Son 30 gün tablo
    if cashflow_list:
        df_cf = pd.DataFrame(cashflow_list).copy()
        df_cf["net"] = df_cf["income"].fillna(0) - df_cf["expense"].fillna(0)
        df_cf = df_cf.sort_values("date")

        # "bir önceki gün" (takvim günü) mantığı için: tarih aralığını tam üret
        rng = pd.date_range(start=start_day, end=today, freq="D")
        df_full = pd.DataFrame({"date": rng.date.astype(str)}).merge(df_cf, on="date", how="left")
        df_full["income"] = df_full["income"].fillna(0.0)
        df_full["expense"] = df_full["expense"].fillna(0.0)
        df_full["net"] = df_full["income"] - df_full["expense"]
        df_full["degisim"] = df_full["net"] - df_full["net"].shift(1)
        df_full["degisim"] = df_full["degisim"].fillna(0.0)

        df_show = df_full.rename(columns={
            "date": "Gün",
            "income": "Gelir (TL)",
            "expense": "Gider (TL)",
            "net": "Kazanç/Zarar (TL)",
            "degisim": "Düne Göre Değişim (TL)",
        })[["Gün", "Gelir (TL)", "Gider (TL)", "Kazanç/Zarar (TL)", "Düne Göre Değişim (TL)"]]
...
# Fiyatları Güncelle, Bilançoyu Yükle, Bilançoyu Kaydet, Varlıkları İndir (CSV), Borçları İndir (CSV) Butonları
...
st.sidebar.download_button(
    "Varlıkları İndir (CSV)",
    data=csv_assets,
    file_name="varliklar.csv",
    mime="text/csv",
)
